...
import { checkpointQueries, poolQueries, swapQueries, testConnection } from "./db"
...
class EventIndexer {
  ...
  private async handlePoolCreated(event: SuiEvent) {
    ...
    // biome-ignore lint/suspicious/noExplicitAny: Sui pool object fields are dynamically typed
    ...
    await poolQueries.upsert({
      ...
      fee_rate: parseInt(fields.fee_bps || "30", 10),
      protocol_fee: parseInt(fields.protocol_fee_bps || "0", 10),
    })
  }
  ...
  private async handleSwap(event: SuiEvent) {
    ...
    console.log(`Swap in pool ${data.pool_id}: ${data.amount_in} → ${data.amount_out}`)
    ...
    await swapQueries.create({
      ...
      timestamp: new Date(parseInt(event.timestampMs || "0", 10)),
    })
  ...
  private async handleLiquidityAdded(event: SuiEvent) {
    ...
    console.log(`Liquidity added to pool ${data.pool_id}: ${data.lp_tokens} LP`)
  ...
  private async handleLiquidityRemoved(event: SuiEvent) {
    const data = event.parsedJson as LiquidityEvent
    console.log(`Liquidity removed from pool ${data.pool_id}: ${data.lp_tokens} LP`)
