...
function calculateProtocolAnalytics(pools: any[]): ProtocolAnalytics {
  ...
  for (const pool of pools) {
    const reserveX = parseFloat(pool.reserve_x) / Math.pow(10, 9); // Assume 9 decimals for SUI
    const reserveY = parseFloat(pool.reserve_y) / Math.pow(10, 9);
...
function calculatePoolAnalytics(pool: any): PoolAnalytics {
  const reserveX = parseFloat(pool.reserve_x) / Math.pow(10, 9);
  const reserveY = parseFloat(pool.reserve_y) / Math.pow(10, 9);
