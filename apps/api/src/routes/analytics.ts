 ...
 */
function calculateProtocolAnalytics(pools: PoolData[]): ProtocolAnalytics {
  let totalTvl = 0
...
function calculatePoolAnalytics(pool: PoolData): PoolAnalytics {
...
export const createAnalyticsPlugin = new Elysia({ prefix: "/api/analytics" })
  ...
  .get("/:id", async ({ params }) => {
    try {
      ...
      let pool: Awaited<ReturnType<typeof poolQueries.getById>>
      try {
