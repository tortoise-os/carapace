...
function generateMockTransactions(address: string, limit: number, offset: number): Transaction[] {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    const type = types[index % types.length] ?? "swap"
    const tokenIn = tokens[index % tokens.length] ?? "SUI"
    const tokenOut = tokens[(index + 1) % tokens.length] ?? "USDC"
...
export const createTransactionHistoryPlugin = (_client: SuiClient) => {
  return (
