...
export function LiquidityDepthChart({ poolId, tokenPair = "SUI/USDC" }: LiquidityDepthChartProps) {
  ...
  useEffect(() => {
    const fetchLiquidityDepth = async () => {
      try {
        const response = await fetch(`http://localhost:3500/api/pools/${poolId}/liquidity-depth`)
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-background border rounded-lg p-3 shadow-lg">
          ...
          {bidLiq !== null && <p className="text-green-600">Buy: ${bidLiq.toLocaleString()}</p>}
          {askLiq !== null && <p className="text-red-600">Sell: ${askLiq.toLocaleString()}</p>}
        </div>
      )
    }
    return null
