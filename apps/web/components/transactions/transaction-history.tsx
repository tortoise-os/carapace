...
export function TransactionHistory({ poolId, limit }: TransactionHistoryProps) {
	const { transactions, loading, error, refetch, hasNextPage, loadMore } =
		useTransactionHistory({
			poolId,
			limit,
		});
				...
				<div className="mt-2 space-y-1 text-xs">
					{transaction.amounts.tokenX && (
						<div>
							...
							<span className="font-medium">
								{formatAmount(transaction.amounts.tokenX)}
							</span>
						</div>
					)}
					{transaction.amounts.tokenY && (
						<div>
							...
							<span className="font-medium">
								{formatAmount(transaction.amounts.tokenY)}
							</span>
						</div>
					)}
					{transaction.amounts.liquidity && (
						<div>
							...
							<span className="font-medium">
								{formatAmount(transaction.amounts.liquidity)}
							</span>
						</div>
					)}
					{transaction.fee && (
