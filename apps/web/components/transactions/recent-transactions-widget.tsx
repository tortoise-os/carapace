      ...
      {transactions.map((tx) => (
        <div key={tx.id} className="flex items-center gap-2 p-2 rounded-lg bg-muted/30 text-xs">
          ...
          <span className="flex-1 truncate">
            {tx.tokenIn && tx.tokenOut ? `${tx.tokenIn} → ${tx.tokenOut}` : "Transaction"}
          </span>
          {tx.status === "success" && <CheckCircle className="h-3 w-3 text-green-600" />}
          {tx.status === "failed" && <XCircle className="h-3 w-3 text-red-600" />}
          {tx.status === "pending" && <Clock className="h-3 w-3 text-yellow-600" />}
        </div>
      ))}
    </div>
  )
