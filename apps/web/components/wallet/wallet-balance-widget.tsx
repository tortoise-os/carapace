...
export function WalletBalanceWidget() {
  ...
  const { tokens, loading: tokensLoading, refresh } = useTokens({ enabled: true })
...
function TokenBalanceRow({
  ...
  const usdValue = formattedBalance && price ? parseFloat(formattedBalance) * price : 0

  return (
    <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
      <div className="flex items-center gap-3">
        <TokenIcon symbol={symbol} size="sm" gradient="bg-brand-gradient-ocean" />
        <div>
      ...
      <div className="text-right">
        ...
        {price && <div className="text-xs text-muted-foreground">${price.toFixed(2)}</div>}
      </div>
    </div>
  )
}

// biome-ignore lint/suspicious/noExplicitAny: Token type varies based on data source
  ...
  return (
    <div className="mt-4 pt-4 border-t">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <div className="text-2xl font-bold">{formatUSD(calculateTotal())}</div>
...
export function CompactWalletBalance() {
  ...
  const usdValue = formattedBalance && suiPrice ? parseFloat(formattedBalance) * suiPrice : 0
    ...
    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/50">
      ...
      <div>
        <div className="text-xs font-medium">
          {isLoading ? "..." : `${parseFloat(formattedBalance || "0").toFixed(2)} SUI`}
        </div>
        <div className="text-[10px] text-muted-foreground">{formatUSD(usdValue)}</div>
      </div>
    </div>
  )
}
