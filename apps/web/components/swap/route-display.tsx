...
import { Badge, Card } from "@carapace/carapace-ui"
import { ArrowRight, Info, TrendingDown, TrendingUp, Zap } from "lucide-react"
import type { SwapRoute } from "@/lib/services/route-optimizer"
  ...
  return (
    ...
    >
        ...
        {route.path.map((token, index) => (
          <div key={token} className="flex items-center gap-2">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-accent/10 border border-accent/20">
              ...
              <span className="text-sm font-semibold">{getTokenSymbol(token)}</span>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
        ...
        <div>
          <div className="text-xs text-muted-foreground mb-1">Price Impact</div>
          <div
        ...
        <div>
          ...
          <div className="text-sm font-bold">{parseFloat(route.totalFees).toFixed(4)}</div>
        </div>
      </div>
      ...
      {showComparison && showComparison.savingsPercent > 0 && (
        <div className="flex items-center gap-2 p-2 rounded-lg bg-green-100 dark:bg-green-900/20 border border-green-200 dark:border-green-800">
          ...
          <span className="text-sm font-semibold text-green-700 dark:text-green-400">
            +{showComparison.savingsPercent.toFixed(2)}% better than direct swap
          </span>
        </div>
      )}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs">
                    {getTokenSymbol(hop.tokenIn)} → {getTokenSymbol(hop.tokenOut)}
                  </span>
                </div>
                <div className="text-xs text-muted-foreground">
                  {parseFloat(hop.amountOut).toFixed(4)} {getTokenSymbol(hop.tokenOut)}
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="space-y-3">
      ...
      {routes.map((route, index) => {
        ...
        if (directRoute && route.hops.length > 1) {
          ...
          const savingsPercent = ((multiHopOutput - directOutput) / directOutput) * 100
          showComparison = {
