...
export default function GovernancePage() {
  ...
  const [governanceToken, setGovernanceToken] = useState<GovernanceToken | null>(null)
  const [selectedStatus, setSelectedStatus] = useState<"all" | ProposalStatus>("all")
  const [_selectedProposal, _setSelectedProposal] = useState<Proposal | null>(null)
  ...
  const getStatusBadge = (status: ProposalStatus) => {
    const variants: Record<ProposalStatus, { bg: string; text: string; label: string }> = {
      pending: {
  ...
  const filteredProposals =
    selectedStatus === "all" ? proposals : proposals.filter((p) => p.status === selectedStatus)
                    ...
                    <div className="space-y-3 mb-4">
                      ...
                      <div>
                        <div className="flex justify-between text-sm mb-1">
                          ...
                          <span className="text-muted-foreground">
                            {formatNumber(proposal.votesFor)} ({results.forPercent.toFixed(1)}%)
                          </span>
                        </div>
                      ...
                      <div className="pt-2">
                        <div className="flex justify-between text-xs text-muted-foreground mb-1">
                          <span>Quorum: {proposal.quorum}% required</span>
                          <span>{results.quorumProgress.toFixed(2)}% reached</span>
                        </div>
                        <div className="h-1 bg-muted rounded-full overflow-hidden">
                          <div
                            className={`h-full ${
                              results.quorumProgress >= parseFloat(proposal.quorum)
                                ? "bg-green-600"
                                : "bg-yellow-600"
                            }`}
                            style={{
                      ...
                      <div className="bg-muted p-3 rounded-lg">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm font-semibold">
                            You voted {proposal.userVote.choice.toUpperCase()} with{" "}
                            {formatNumber(proposal.userVote.power)} voting power
                          </span>
                        </div>
                      </div>
                    )}
