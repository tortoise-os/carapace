...
import { Droplet, Plus, Star, TrendingUp } from "lucide-react"
...
export default function PoolsPage() {
  ...
  const filteredPools = showFavoritesOnly ? pools.filter((pool) => isFavorite(pool.pool_id)) : pools
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {[
            {
              ...
              value: "$12.5M",
              icon: Droplet,
              gradient: "bg-accent-gradient-blue",
              cardGradient: "from-accent-blue/30 to-accent-indigo/25",
            }, // Water, liquidity - BLUE
            {
              label: "Total Volume (24h)",
              value: "$5.8M",
              icon: TrendingUp,
              gradient: "bg-accent-gradient-orange",
              cardGradient: "from-accent-orange/30 to-accent-amber/25",
            }, // Activity, heat - ORANGE
            {
              label: "Total Pools",
              value: "12",
              icon: Plus,
              gradient: "bg-accent-gradient-purple",
            ...
            <BlurFade key={stat.label} delay={0.2 + i * 0.1}>
              ...
              >
                <div
                  className={`w-14 h-14 rounded-xl ${stat.gradient} flex items-center justify-center mb-4 shadow-md group-hover:scale-110 transition-transform duration-200`}
                >
                  <stat.icon className="w-7 h-7 text-white" />
                </div>
                <div className="text-sm text-muted-foreground font-semibold mb-1 uppercase tracking-wide">
                  {stat.label}
                </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          {filteredPools.map((pool, i) => {
            ...
            const tvl = (
              (Number(pool.reserve_x) / 1e9) * 1.5 +
              Number(pool.reserve_y) / 1e6
            ).toFixed(2)
                ...
                <MagicCard className="p-6 card group/card">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-wrap gap-8 group-hover/card:scale-[1.01] transition-transform">
                      ...
                      <div>
                        <div className="text-sm text-muted-foreground font-semibold mb-1 uppercase tracking-wide">
                          Reserve {tokenXSymbol}
                        </div>
                        <div className="text-lg font-bold text-foreground">
                          {(Number(pool.reserve_x) / 1e9).toFixed(2)}
                        </div>
                      </div>
                      <div>
                        <div className="text-sm text-muted-foreground font-semibold mb-1 uppercase tracking-wide">
                          Reserve {tokenYSymbol}
                        </div>
                        <div className="text-lg font-bold text-foreground">
                          {(Number(pool.reserve_y) / 1e6).toFixed(2)}
                        </div>
                      </div>
                    </div>
