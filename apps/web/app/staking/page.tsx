...
export default function StakingPage() {
  ...
  useEffect(() => {
    const loadData = () => {
      ...
      if (account?.address) {
        const userPositions =
          liquidityMiningService.getUserStakingPositions(account.address);
        const userStats =
          liquidityMiningService.getUserStakingStats(account.address);
        const history =
          liquidityMiningService.getUserClaimHistory(account.address);
  ...
  const handleStake = () => {
    ...
    try {
      ...
      const position = liquidityMiningService.stake(params, account.address);
  ...
  const handleClaimRewards = (positionId: string) => {
    ...
    try {
      ...
      toast.success(
        `Successfully claimed ${result.rewards} ${result.position.rewardTokenSymbol}!`
      );
          ...
          <div className="flex gap-2 mb-6 border-b">
            {[
              ...
              <Button
                ...
                onClick={() =>
                  setSelectedTab(tab.key as typeof selectedTab)
                }
                        ...
                        {position.status === 'active' && (
                          <>
                            <Button
                              ...
                              disabled={
                                parseFloat(position.rewardsEarned) <= 0
                              }
                            >
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-4">
                        <div className="text-right">
                          <p className="font-semibold">
                            {formatCurrency(claim.rewardValueUSD)}
                          </p>
                        </div>
                        <Button
                          variant="ghost"
              ...
              <Card className="max-w-lg w-full p-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-bold">
                    Stake in {selectedProgram.poolName}
                  </h2>
                  <Button
                    variant="ghost"
                    size="sm"
                  ...
                  <Card className="p-4 bg-green-50 dark:bg-green-950 border-green-200 dark:border-green-800">
                    ...
                    <div className="text-3xl font-bold text-green-600">
                      {(
                        ...
                        (LOCK_PERIOD_OPTIONS.find(
                          (o) => o.days === selectedLockPeriod
                        )?.aprMultiplier || 1)
                      ).toFixed(1)}
                      %
                    </div>
