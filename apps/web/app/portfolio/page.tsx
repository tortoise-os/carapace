...
import { BlurFade, Button, Card, DotPattern, MagicCard } from "@carapace/carapace-ui"
...
export default function PortfolioPage() {
  ...
  const [selectedTimeframe, setSelectedTimeframe] = useState<"day" | "week" | "month" | "all">(
    "day"
  )
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      setIsLoading(true)
      try {
        const portfolioData = await portfolioService.getPortfolio(account.address)
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <div className="flex items-center gap-4">
                  ...
                  >
                    ...
                    <span className="text-sm">({formatPercent(pnl.percent)})</span>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-right">
                  ...
                  <p className="text-2xl font-bold">{portfolio.stats.totalTokens}</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-muted-foreground mb-1">LP Positions</p>
                  <p className="text-2xl font-bold">{portfolio.stats.totalPositions}</p>
                </div>
              </div>
            </div>
          </MagicCard>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {portfolio.stats.topGainer && (
                <Card className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Top Gainer</p>
                      <p className="text-lg font-bold">{portfolio.stats.topGainer.token}</p>
                    </div>
                    <div className="text-right">
              ...
              {portfolio.stats.topLoser && (
                <Card className="p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-muted-foreground mb-1">Top Loser</p>
                      <p className="text-lg font-bold">{portfolio.stats.topLoser.token}</p>
                    </div>
                    <div className="text-right">
                ...
                <div className="flex-1">
                  ...
                  <ul className="space-y-2">
                    {suggestions.map((suggestion, _i) => (
                      <li key={suggestion.message} className="text-sm text-muted-foreground">
                        • {suggestion.message}
                      </li>
                    ))}
                  </ul>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-semibold">{allocation.tokens.toFixed(1)}%</span>
                </div>
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-primary" style={{ width: `${allocation.tokens}%` }} />
                </div>
              ...
              <div>
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className="font-semibold">{allocation.liquidity.toFixed(1)}%</span>
                </div>
                <div className="h-2 bg-muted rounded-full overflow-hidden">
                  <div className="h-full bg-accent" style={{ width: `${allocation.liquidity}%` }} />
                </div>
                <p className="text-sm text-muted-foreground mt-1">
            ...
            <div className="space-y-3">
              {portfolio.tokenBalances.map((token, _index) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className="font-semibold">{formatCurrency(token.valueUSD)}</p>
            ...
            <Card className="p-6 mb-8">
              ...
              <div className="space-y-3">
                {portfolio.liquidityPositions.map((position, _index) => (
                  <div key={position.poolId} className="p-4 bg-muted/30 rounded-lg">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-semibold">{formatCurrency(position.feesEarned24h)}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground mb-1">Total Fees</p>
                        <p className="font-semibold">{formatCurrency(position.feesEarnedTotal)}</p>
                      </div>
                    </div>
                  </div>
                ))}
                    ...
                    <div className="text-right">
                      {tx.tokens.map((token, _i) => (
                        <p
                          key={token.symbol}
                          className={`text-sm ${
                            token.amount.startsWith("-") ? "text-red-600" : "text-green-600"
                          }`}
                        >
