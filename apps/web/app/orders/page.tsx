...
import { Badge, BlurFade, Button, Card, DotPattern, MagicCard } from "@carapace/carapace-ui"
...
import type { LimitOrder, OrderStatus } from "@/lib/types/limit-orders"
    ...
    <div className="relative min-h-screen py-12">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12">
          <BlurFade delay={0.1}>
            <div>
              <h1 className="text-4xl md:text-5xl font-bold mb-2">Limit Orders</h1>
        ...
        <BlurFade delay={0.4}>
          <div className="flex gap-2 mb-6 overflow-x-auto">
            {(["all", "pending", "filled", "cancelled", "expired"] as const).map((status) => (
              <Button
                key={status}
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          ...
                          <div className="font-semibold">
                            {parseFloat(order.amountIn).toFixed(2)} {getTokenSymbol(order.tokenIn)}
                          </div>
                        </div>

                        <div>
                          <div className="text-muted-foreground mb-1">Target Price</div>
                          <div className="font-semibold">
                        ...
                        <div>
                          ...
                          <div
                            className={`font-semibold flex items-center gap-1 ${
                              parseFloat(order.currentPrice) > parseFloat(order.targetPrice)
                                ? "text-green-600"
                                : "text-red-600"
                            }`}
                          >
                        ...
                        {order.status === "pending" && (
                          <div>
                            ...
                            <div className="font-semibold">{getTimeRemaining(order.expiresAt)}</div>
                          </div>
                        )}

                        {order.status === "filled" && (
                          <div>
                            <div className="text-muted-foreground mb-1">Filled Price</div>
                            <div className="font-semibold text-green-600">{order.filledPrice}</div>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {order.status === "pending" && (
                      <div>
                        <Button
                          ...
                          size="sm"
                          onClick={() => {
                            limitOrdersService.cancelOrder(order.id, account.address)
                            // Refresh orders
                          }}
                        >
                          Cancel
