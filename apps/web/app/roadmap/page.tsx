...
export default function RoadmapPage() {
	...
	const p1Progress =
		(p1Features.filter((f) => f.status === "completed").length /
			p1Features.length) *
		100;
	const p2Progress =
		(p2Features.filter((f) => f.status === "completed").length /
			p2Features.length) *
		100;
	const p3Progress =
		(p3Features.filter((f) => f.status === "completed").length /
			p3Features.length) *
		100;
	const overallProgress =
		(ROADMAP.filter((f) => f.status === "completed").length / ROADMAP.length) *
		100;
				...
				<BlurFade delay={0.1}>
					<div className="text-center mb-12">
						...
						<p className="text-xl text-muted-foreground max-w-2xl mx-auto">
							Building the future of DeFi on Sui - Track our journey from
							concept to reality
						</p>
					</div>
				</BlurFade>
