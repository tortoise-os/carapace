...
test.describe("Swap Interface", () => {
	...
	test("should display swap interface", async ({ page }) => {
		...
		await expect(
			page.getByRole("button", { name: /SUI/ }).first(),
		).toBeVisible();
		await expect(
	...
	test("should open settings dialog when clicking settings button", async ({
		...
		await expect(
			page.getByRole("heading", { name: "Swap Settings" }),
		).toBeVisible();
		...
		await expect(
			page.getByText("Block Explorer", { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText("Gas Budget (Optional)", { exact: true }),
		).toBeVisible();
		await expect(
			page.getByText("Using a Ledger", { exact: true }),
		).toBeVisible();
	});

	test("should open token selector when clicking token button", async ({
		page,
	}) => {
		...
		await expect(page.getByRole("dialog")).toBeVisible();
		await expect(
			page.getByRole("heading", { name: "Select a token" }),
		).toBeVisible();
	...
	test("should display price impact badge in swap details", async ({
		page,
	}) => {
		// Enter amount
		await page.getByPlaceholder("0").first().fill("10");
	...
	test("should show price impact warning for medium impact swaps", async ({
		page,
	}) => {
