...
test.describe('Analytics Dashboard', () => {
  ...
  test('should display analytics page', async ({ page }) => {
    ...
    // Check description
    await expect(page.getByText('Protocol-wide statistics and pool performance metrics')).toBeVisible();
  });
  ...
  test('should display pool analytics table', async ({ page }) => {
    ...
    // Check table heading
    await expect(page.getByRole('heading', { name: 'Pool Analytics' })).toBeVisible();
  ...
  test('should sort by volume when clicking volume header', async ({ page }) => {
    // Wait for data to load
    await page.waitForTimeout(1500);
  ...
  test('should refresh data automatically', async ({ page }) => {
    ...
    const tvlCard = page.locator('text=Total Value Locked').locator('..');
    const initialValue = await tvlCard.textContent();
  ...
  test('should display loading state initially', async ({ page }) => {
    ...
    const statsSection = page.locator('text=Total Value Locked').locator('../..');
