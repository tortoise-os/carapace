 ...
 */
export function useTokenMetadata(coinType: string | undefined): UseTokenMetadataResult {
  ...
  useEffect(() => {
    ...
    async function fetchMetadata() {
      try {
        ...
        // biome-ignore lint/style/noNonNullAssertion: Value guaranteed to exist at this point
  ...
  useEffect(() => {
    ...
    async function fetchMetadata() {
      try {
        ...
        const promises = coinTypes.map((coinType) => sdk.tokenMetadata.getTokenMetadata(coinType))
        ...
        if (!cancelled) {
          ...
          results.forEach((metadata, index) => {
            // biome-ignore lint/style/noNonNullAssertion: results and coinTypes arrays have same length
        ...
        if (!cancelled) {
          setError(err instanceof Error ? err : new Error("Failed to fetch tokens metadata"))
        }
      } finally {
        if (!cancelled) {
          setLoading(false)
