...
export function useTransactionHistory(
  options: { limit?: number; poolId?: string; type?: string; enabled?: boolean } = {}
  ...
  useEffect(() => {
    fetchTransactions()
    // biome-ignore lint/correctness/useExhaustiveDependencies: fetchTransactions intentionally not wrapped
  }, [fetchTransactions])
...
export function usePoolTransactionHistory(
  ...
  const fetchTransactions = async (loadMore = false) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3501/transactions/pool/${poolId}?${params}`)
      ...
      setError(err instanceof Error ? err : new Error("Failed to fetch pool transaction history"))
    } finally {
      setLoading(false)
    }
  }
