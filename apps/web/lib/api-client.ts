...
class ApiClient {
  ...
  async getPools(limit = 100, offset = 0): Promise<Pool[]> {
    const response = await fetch(`${this.baseUrl}/api/pools?limit=${limit}&offset=${offset}`)
    if (!response.ok) {
  ...
  async getSwapQuote(poolId: string, amountIn: string, isXToY: boolean): Promise<SwapQuote> {
  ...
  async getPoolAnalytics(params?: {
    ...
    const response = await fetch(`${this.baseUrl}/api/analytics/pools?${queryParams.toString()}`)
    if (!response.ok) {
  ...
  async calculateRemoveLiquidity(params: { poolId: string; lpTokenAmount: string }): Promise<{
    amountX: string
    amountY: string
    feesX: string
    feesY: string
  ...
  async getTokens(params?: { enabled?: boolean; verified?: boolean }): Promise<TokenListResponse> {
    const queryParams = new URLSearchParams()
    ...
    const response = await fetch(`${this.baseUrl}/api/tokens?${queryParams.toString()}`)
    if (!response.ok) {
