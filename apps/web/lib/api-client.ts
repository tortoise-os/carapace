...
class ApiClient {
	...
	async getPosition(
		...
		const response = await fetch(
			`${this.baseUrl}/api/positions/${address}/${poolId}`,
		);
		if (!response.ok) {
	...
	async calculateAddLiquidity(params: {
		...
		const response = await fetch(
			`${this.baseUrl}/api/positions/calculate-add`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	async calculateRemoveLiquidity(params: {
		...
		const response = await fetch(
			`${this.baseUrl}/api/positions/calculate-remove`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
	...
	async getTokens(params?: {
		...
		if (params?.enabled !== undefined)
			queryParams.set("enabled", params.enabled.toString());
	...
	async getTokenByAddress(address: string): Promise<Token | null> {
		const response = await fetch(
			`${this.baseUrl}/api/tokens/address/${address}`,
		);
		if (!response.ok) {
			throw new Error("Failed to fetch token");
		}
