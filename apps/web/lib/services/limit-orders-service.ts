import type { CreateLimitOrderParams, LimitOrder, LimitOrdersFilter } from "../types/limit-orders"
    ...
    for (const [orderId, order] of ordersStorage.entries()) {
      ...
      if (currentPrice !== undefined) {
        ...
        if (this.shouldExecuteOrder(order, currentPrice)) {
          ...
          const txDigest = `0x${Math.random().toString(16).substring(2, 18)}`

          this.executeOrder(orderId, currentPrice.toString(), filledAmount, txDigest)
