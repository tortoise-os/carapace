...
export class LiquidityMiningService {
  ...
  stake(params: StakeParams, userAddress: string): StakingPosition {
    ...
    const lockOption = LOCK_PERIOD_OPTIONS.find((opt) => opt.days === params.lockPeriod)

    if (!lockOption) {
      throw new Error("Invalid lock period")
    }

    const positionId = `stake_${Date.now()}_${Math.random().toString(36).substring(7)}`
  ...
  private calculateRewards(position: StakingPosition): {
    ...
    const rewardsUSD = (position.valueUSD * (position.apr / 100) * daysStaked) / 365
  ...
  getUserStakingStats(userAddress: string): StakingStats {
    ...
    const totalStakedUSD = activePositions.reduce((sum, pos) => sum + pos.valueUSD, 0)

    const totalRewardsEarnedUSD = positions.reduce((sum, pos) => sum + pos.rewardsEarnedUSD, 0)

    const averageAPR =
      activePositions.length > 0
        ? activePositions.reduce((sum, pos) => sum + pos.apr, 0) / activePositions.length
        : 0
   ...
   */
  private addClaimHistory(userAddress: string, position: StakingPosition, rewardAmount: string) {
  ...
  estimateRewards(
    ...
    // Find lock period multiplier
    const lockOption = LOCK_PERIOD_OPTIONS.find((opt) => opt.days === lockPeriod)
