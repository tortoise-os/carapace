...
export class RouteOptimizer {
  ...
  findAllRoutes(
    ...
    return routes.sort((a, b) =>
      parseFloat(b.totalAmountOut) - parseFloat(a.totalAmountOut)
    );
  }
  ...
  private dfs(
    ...
    const availablePools = this.pools.filter(
      (pool) =>
        pool.token_x === currentToken || pool.token_y === currentToken
    );
  ...
  private calculateHop(
    ...
    const reserveIn = isXtoY
      ? BigInt(pool.reserve_x)
      : BigInt(pool.reserve_y);
  ...
  private buildRoute(hops: RouteHop[]): SwapRoute {
    ...
    const totalPriceImpact = hops.reduce(
      (acc, hop) => acc * (1 + hop.priceImpact / 100),
      1
    ) - 1;
    ...
    const totalFees = hops.reduce((acc, hop) => {
      const feeAmount =
        (parseFloat(hop.amountIn) * hop.pool.fee_rate) / 10000;
   ...
   */
  compareRoutes(tokenIn: string, tokenOut: string, amountIn: string): {
    ...
    const bestMultiHopRoute = multiHopRoutes.length > 0 ? multiHopRoutes[0] : null;
