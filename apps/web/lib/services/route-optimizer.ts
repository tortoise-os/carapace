...
export class RouteOptimizer {
   ...
   */
  findAllRoutes(tokenIn: string, tokenOut: string, amountIn: string): SwapRoute[] {
    ...
    return routes.sort((a, b) => parseFloat(b.totalAmountOut) - parseFloat(a.totalAmountOut))
  }

  /**
   * Find the optimal route with best output
   */
  findBestRoute(tokenIn: string, tokenOut: string, amountIn: string): SwapRoute | null {
  ...
  private dfs(
    ...
    for (const pool of availablePools) {
      const nextToken = pool.token_x === currentToken ? pool.token_y : pool.token_x
      ...
      if (hop) {
        path.push(hop)
        this.dfs(nextToken, targetToken, hop.amountOut, path, visited, routes, depth + 1)
        path.pop()
      }
    }
  ...
  private calculateHop(
    ...
    const amountInWithFee = (amountInBigInt * (BigInt(10000) - feeRate)) / BigInt(10000)
    ...
    const priceImpact =
      (Number(amountInWithFee) / (Number(reserveIn) + Number(amountInWithFee))) * 100
  ...
  private buildRoute(hops: RouteHop[]): SwapRoute {
    ...
    const totalPriceImpact = hops.reduce((acc, hop) => acc * (1 + hop.priceImpact / 100), 1) - 1
  ...
  compareRoutes(
    ...
    const directRoute = allRoutes.find((route) => route.hops.length === 1) || null
