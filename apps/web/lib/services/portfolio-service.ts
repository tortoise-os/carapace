...
export class PortfolioService {
   ...
   */
  private async fetchTokenBalances(_userAddress: string): Promise<TokenBalance[]> {
   ...
   */
  private async fetchLiquidityPositions(_userAddress: string): Promise<LiquidityPosition[]> {
   ...
   */
  private async fetchPortfolioHistory(_userAddress: string): Promise<PortfolioHistory[]> {
  ...
  private calculateStats(
    ...
    const tokenBalanceValue = tokenBalances.reduce((sum, token) => sum + token.valueUSD, 0)
    const liquidityPositionValue = liquidityPositions.reduce((sum, pos) => sum + pos.valueUSD, 0)
    ...
    const sortedByChange = [...tokenBalances].sort((a, b) => b.change24h - a.change24h)
   ...
   */
  private calculatePnL(history: PortfolioHistory[], currentValue: number): PortfolioPnL {
    ...
    const monthAgoValue = history.length >= 30 ? getDayAgoValue(30) : currentValue
    const firstHistoryItem = history[0]
    const firstValue = firstHistoryItem ? firstHistoryItem.totalValue : currentValue
   ...
   */
  async getTransactions(_userAddress: string, limit: number = 20): Promise<PortfolioTransaction[]> {
