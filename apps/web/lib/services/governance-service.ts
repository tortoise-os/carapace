...
export class GovernanceService {
   ...
   */
  createProposal(params: CreateProposalParams, proposer: string): Proposal {
  ...
  getProposals(filter?: ProposalFilter): Proposal[] {
    ...
    // biome-ignore lint/suspicious/useIterableCallbackReturn: updateProposalStatus returns void implicitly
    allProposals.forEach((p) => this.updateProposalStatus(p))

    if (filter) {
      if (filter.status) {
        allProposals = allProposals.filter((p) => filter.status?.includes(p.status))
      }
      if (filter.type) {
        allProposals = allProposals.filter((p) => filter.type?.includes(p.type))
      }
      if (filter.proposer) {
        allProposals = allProposals.filter((p) => p.proposer === filter.proposer)
      }
    }
   ...
   */
  private getVotingPower(_userAddress: string): string {
  ...
  private updateProposalStatus(proposal: Proposal): void {
    ...
    if (proposal.status === "active" && now >= proposal.endTime) {
      ...
      const quorumReached =
        (parseFloat(proposal.totalVotes) / totalSupply) * 100 >= parseFloat(proposal.quorum)

      const totalVotes = parseFloat(proposal.totalVotes)
      const thresholdReached =
        totalVotes > 0 &&
        (parseFloat(proposal.votesFor) / totalVotes) * 100 >= parseFloat(proposal.threshold)
