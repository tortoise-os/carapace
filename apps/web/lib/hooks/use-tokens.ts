...
export function useTokens(options: UseTokensOptions = {}): UseTokensReturn {
  const { enabled, verified, autoRefresh = false, refreshInterval = 30000 } = options
  ...
  useEffect(() => {
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: startPolling intentionally not wrapped
