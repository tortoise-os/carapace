...
export function useTokenBalance(
  ...
  const [coins, setCoins] = useState<Array<{ objectId: string; balance: bigint }>>([])
  ...
  const fetchBalance = async () => {
    ...
    try {
      ...
      const tokenBalance = await sdk.balance.getTokenBalance(account.address, tokenAddress)
  ...
  useEffect(() => {
    fetchBalance()
    // biome-ignore lint/correctness/useExhaustiveDependencies: fetchBalance intentionally not wrapped
  }, [fetchBalance])
...
export function useTokenBalances(
  ...
  useEffect(() => {
    fetchBalances()
    // biome-ignore lint/correctness/useExhaustiveDependencies: fetchBalances intentionally not wrapped
  }, [fetchBalances])
