...
export function useTokenBalance(
  ...
  useEffect(() => {
    fetchBalance();
  }, [account?.address, tokenAddress, enabled]);
  ...
  }, [account?.address, tokenAddress, enabled, refreshInterval]);
...
export function useTokenBalances(
  ...
  useEffect(() => {
    fetchBalances();
  }, [account?.address, enabled, JSON.stringify(tokens.map(t => t.address))]);
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [account?.address, enabled, refreshInterval, JSON.stringify(tokens.map(t => t.address))]);
...
export function useAllTokenBalances(
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchBalances, refreshInterval);
    return () => clearInterval(interval);
  }, [account?.address, enabled, refreshInterval]);
