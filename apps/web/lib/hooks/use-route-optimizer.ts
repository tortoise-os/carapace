...
import { RouteOptimizer, SwapRoute } from '../services/route-optimizer';
...
export function useRouteOptimizer(
  ...
  useEffect(() => {
    if (!optimizer || !tokenIn || !tokenOut || !amountIn || parseFloat(amountIn) <= 0) {
      setRoutes([]);
      return;
    }
    ...
    const timer = setTimeout(() => {
      try {
        const foundRoutes = optimizer.findAllRoutes(tokenIn, tokenOut, amountIn);
