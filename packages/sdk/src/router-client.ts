...
export class RouterClient {
  ...
  buildMultiHopSwap(
    ...
    const minOutput = (route.expectedOutput * BigInt(10000 - slippageBps)) / BigInt(10000);
  ...
  private calculateRouteOutput(
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      currentAmount = this.getAmountOut(currentAmount, reserveIn, reserveOut, feeBps);
    }

    // Calculate price impact
    const priceImpact = this.calculatePriceImpact(amountIn, currentAmount, path, pools);
  ...
  private calculatePriceImpact(
    ...
    if (path.length === 2) {
      ...
      const impact = ((effectivePrice - spotPriceBefore) / spotPriceBefore) * 100;
