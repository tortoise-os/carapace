...
export class PoolClient {
	...
	async getSwapQuote(
		...
		const fee =
			(amountIn * BigInt(pool.feeBps)) / BigInt(CONSTANTS.BPS_DENOMINATOR);
	...
	async getSpotPrice(poolId: ObjectId): Promise<number> {
		const pool = await this.getPool(poolId);
		return (
			Number((pool.reserveY * CONSTANTS.PRICE_PRECISION) / pool.reserveX) /
			Number(CONSTANTS.PRICE_PRECISION)
		);
	}
	...
	removeLiquidity(
		...
		const [lpSplit] = tx.splitCoins(tx.object(lpToken), [
	...
	swapXToY(
		...
		const coinOut = tx.moveCall({
			...
			typeArguments: [typeX, typeY],
			arguments: [tx.object(poolId), coinSplit, tx.pure.u64(minAmountOut)],
		});
