...
export class GasEstimator {
	 ...
	 */
	async estimateSwapGas(tx: Transaction, sender: string): Promise<GasEstimate> {
	...
	async compareRouteGas(
		...
		return estimates.sort((a, b) =>
			Number(a.estimate.totalGas - b.estimate.totalGas),
		);
	}
	...
	async estimateBatchGas(
		...
		const total: GasEstimate = {
			...
			estimatedCostSui: (
				Number(individual.reduce((sum, est) => sum + est.totalGas, 0n)) / 1e9
			).toFixed(6),
		};
