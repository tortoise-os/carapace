...
async function main() {
	...
	// Check TEST balance
	const testCoins = await sdk.client.getCoins({
		owner: address,
		coinType: TEST_TYPE,
	});
	const testBalance = testCoins.data.reduce(
		(sum, coin) => sum + BigInt(coin.balance),
		0n,
	);
	...
	if (testBalance < 10_000_000n) {
		console.error(
			"❌ Error: Insufficient TEST balance (need at least 10 TEST)",
		);
		console.log(
			'Run: SUI_PRIVATE_KEY="..." bun run scripts/mint-test-coins.ts',
		);
		process.exit(1);
	}
	...
	try {
		...
		const lpToken = result.objectChanges?.find(
			(change: any) =>
				change.type === "created" && change.objectType.includes("::coin::Coin"),
		);

		if (lpToken && "objectId" in lpToken) {
			console.log(`🪙 LP Token ID: ${lpToken.objectId}`);
			console.log(
				`🔗 LP Token on explorer: https://testnet.suivision.xyz/object/${lpToken.objectId}\n`,
			);
		}
