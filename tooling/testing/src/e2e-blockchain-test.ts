...
async function main() {
	...
	try {
		...
		console.log("\n📝 Test 1: Create Pool");
		console.log("-".repeat(60));
		const createTx = sdk.pool.createPool(SUI_TYPE, SUI_TYPE, {
		...
		if (createResult.effects?.status?.status !== "success") {
			throw new Error(
				`Pool creation failed: ${createResult.effects?.status?.error}`,
			);
		}
		...
		logTest("Create Pool", true, undefined, {
			poolId,
		...
		logTest(
			"Verify Empty Pool",
			isEmptyCorrect,
			isEmptyCorrect ? undefined : "Pool not empty",
			{
		...
		if (addLiqResult.effects?.status?.status !== "success") {
			throw new Error(
				`Add liquidity failed: ${addLiqResult.effects?.status?.error}`,
			);
		}
		...
		const liquidityEvent = addLiqResult.events?.find((e: any) =>
		...
		logTest(
			"Verify Pool After Liquidity",
			isLiquidityCorrect,
			isLiquidityCorrect ? undefined : "Reserves mismatch",
			{
		...
		logTest(
			"Get Swap Quote",
			isQuoteValid,
			isQuoteValid ? undefined : "Invalid quote",
			{
		...
		const swapEvent = swapResult.events?.find((e: any) =>
			e.type.includes("Swapped"),
		);
		...
		logTest(
			"Verify Pool After Swap",
			isSwapCorrect,
			isSwapCorrect ? undefined : "Reserves mismatch after swap",
			{
		...
		logTest(
			...
			isPriceValid,
			isPriceValid ? undefined : "Invalid price",
			{
		...
		if (removeLiqResult.effects?.status?.status !== "success") {
			throw new Error(
				`Remove liquidity failed: ${removeLiqResult.effects?.status?.error}`,
			);
		}

		const removeLiqEvent = removeLiqResult.events?.find((e: any) =>
			e.type.includes("LiquidityRemoved"),
		);
		...
		const reservesReduced =
			poolAfterRemoval.reserveX < poolAfterSwap.reserveX &&
			poolAfterRemoval.reserveY < poolAfterSwap.reserveY;

		const isRemovalCorrect = lpSupplyReduced && reservesReduced;

		logTest(
			"Verify Pool After Removal",
			isRemovalCorrect,
			isRemovalCorrect ? undefined : "Liquidity not removed correctly",
			{
